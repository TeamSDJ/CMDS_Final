...
#title_data,cat_data,text_data = sites_data[['stitle']], sites_data[['stitle']], pd.DataFrame(wiki_links)
def analysis(pd,title_data,cat_data,text_data,hfc=0.,lfc=0.):
    ...
    #print(type(body_vecs_M))
    #print(np.array(weights.tolist()))
    #weighted_doc_vec_M=body_vecs_M*np.array(weights.tolist())

    #weighted_doc_vec_M=body_vecs_M.toarray()*np.array(weights.tolist())
    #print(weighted_doc_vec_M)
    ...
    #weighted_doc_vec_table = pd.DataFrame(np.matrix(weighted_doc_vec_M.toarray()),columns=site_names,index=V)
    doc_vec_table = pd.DataFrame(body_vecs_M.toarray(),columns=site_names,index=V)
